        ...
        for retry_count in range(max_retries):
            try:
                with self.db_connection.engine_mysql_bi.connect() as connection:
                    ...
                    # Agregar logs para la depuración
                    logging.debug(f"Ejecutando consulta SQL: {self.txSql}")
                    logging.debug(f"Parámetros: fi={self.IdtReporteIni}, ff={self.IdtReporteFin}")
                ...
                logging.error(f"Error en consulta_sql_bi (Intento {retry_count + 1}/{max_retries}): {e}")
                time.sleep(1)

        logging.error("Se agotaron los intentos. No se pudo ejecutar la consulta SQL.")
        return 0
